...
document
  .querySelectorAll(".a-text-caps")
  .forEach(
    (item) =>
      (thisOrder["total"] = item
        .closest(".a-fixed-right-grid-col")
        .querySelector("span [class*='order-total']").innerText)
  );

document
  .querySelectorAll(".a-text-caps")
  .forEach((item) =>
    item.innerText == "ORDER PLACED"
      ? orders.push(item.closest(".a-fixed-right-grid-col").innerText)
      : ""
  );

//product name
document
  .querySelectorAll("div[class$='-product-title']")
  .forEach((item) => console.log(item.innerText));

document.querySelectorAll(".a-text-caps").forEach((item) => {
  item.innerText == "ORDER PLACED"
    ? orders.push(item.closest(".a-fixed-right-grid-col").innerText)
    : "",
...
document.querySelectorAll(".order-card").forEach((card) =>
  card.getElementsByClassName("a-text-caps").forEach((item) => {
    item.innerText == "ORDER PLACED"
      ? orders.push(item.closest(".a-fixed-right-grid-col").innerText)
      : "",
...
document
  .querySelectorAll(".order-card")
  .forEach((card) => {
    card.getElementsByClassName("a-text-caps");
  })
  .forEach((item) => console.log(item.innerText))
  .forEach((item) => {
    item.innerText == "ORDER PLACED"
